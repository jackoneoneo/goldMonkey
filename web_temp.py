...
def dataBCChart(databaseName):
    ...
    '''此处路径需要修改'''
    createBCHtml(dirPath+'\\'+databaseName, htmlName)
    return htmlName
...
def dataBmuhart(databaseName,BmuName):
    ...
    htmlName = str(timeInteger)+".html"
    '''此处路径需要修改'''
    createBmuHtml(dirPath+'\\'+databaseName, htmlName,BmuName)
    return htmlName
...
def databaseName():
    ...
    return json.dumps(getFilesName(dirPath))
